...
function sendEmailToManege(orderData) {
  ...
  var mailOptions = {
    ...
    html: `<h2>הזמנה מספר ${numOrder++}</h2><p>שם: ${orderData.user.name}</p><p>אימייל: ${orderData.user.email}</p><p>טלפון: ${orderData.user.phone}</p><p>סכום: ${orderData.order.sumCartAndDelivery}</p><p>מספר מוצרים: ${orderData.order.items.length}</p>`
  };
...
app.post("/order", function(req, res) {
  ...
  MongoClient.connect(url,{
    ...
    dbo.collection("users").findOne(myobj, function(err, result) {
      ...
      if (result) { // if there is email like the query..
        var newvalues = { $set: {phone: req.body.user.phone, address: req.body.user.address } };
...
return res.status(200).send("complete");
      }else{ // if there is no email like the query..
        let newUser ={
          email:req.body.user.email,
          sendEmailes:true,
...
app.post("/login", function(req, res) {
  if (req.body.email && req.body.password) {
    ...
    MongoClient.connect(url,{
      ...
      dbo.collection("admin").findOne(myobj, function(err, result) {
        if (err) {
          return res.sendStatus(500);
        }
        if (result) {
          ...
          console.log(`result is ${result}`);
           ...
           res.send(utils.createToken(result)).status(200);
        }
